...
public class ApplicationConfig {
  ...
  public JwtDecoder jwtDecoder(JWKSource<SecurityContext> source, AuthProp prop) {
    ...
    OAuth2TokenValidator<Jwt> defaultValidator = JwtValidators.createDefaultWithIssuer(prop.getUuid());

    OAuth2TokenValidator<Jwt> expValidator = jwt -> {
      if ((tokenStoreRepo.findByAccessToken(jwt.getTokenValue()).get() == null)
    ...
    OAuth2TokenValidator<Jwt> validator = new DelegatingOAuth2TokenValidator<>(defaultValidator, expValidator);
